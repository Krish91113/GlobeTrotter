...
export const itineraryService = {
    ...
    try {
      ...
      return days.map((d: any, index: number) => ({
        ...
        items: (d.itineraryItems || d.items || []).map((i: any) => ({
          ...
          category:
            i.catalogItem?.categories?.[0] || i.category || "Attractions",
          startTime: i.plannedStartAt
            ? new Date(i.plannedStartAt).toTimeString().slice(0, 5)
            : i.startTime || "09:00",
          endTime: i.plannedEndAt
            ? new Date(i.plannedEndAt).toTimeString().slice(0, 5)
            : i.endTime || "11:00",
          estimatedCost: i.estimatedCost
            ? Number.parseFloat(i.estimatedCost)
            : 0,
          ...
          image: cityImageUrl(
            d.stop?.locationName,
            i.catalogItem?.thumbnailUri || i.image,
          ),
    ...
    try {
      ...
      return stops.map((s: any) => ({
        ...
        arrivalDate: s.arrivalDate
          ? new Date(s.arrivalDate).toISOString().slice(0, 10)
          : "",
    ...
    return apiClient<ItineraryItem>(`/trips/${tripId}/days/${dayId}/items`, {
      ...
      body: JSON.stringify({
        ...
        ...(input.estimatedCost !== undefined && {
          estimatedCost: input.estimatedCost.toFixed(2),
        }),
      }),
    });
  },
  ...
  reorderActivities: async (
    tripId: string,
    dayId: string,
    items: { itemId: string; sequenceNo: number }[],
  ): Promise<void> => {

  ...
  getPublicTrip: async (token: string): Promise<PublicTrip> => {
    ...
    return {
      ...
      budgetSummary: data.estimatedBudget
        ? {
            totalBudget: Number.parseFloat(data.estimatedBudget),
            estimatedSpend: 0,
      ...
      days: (data.days || []).map((d: any, index: number) => ({
        ...
        items: (d.items || []).map((i: any) => ({
          ...
          startTime: i.plannedStartAt
            ? new Date(i.plannedStartAt).toISOString().slice(11, 16)
            : i.startTime || "09:00",
          endTime: i.plannedEndAt
            ? new Date(i.plannedEndAt).toISOString().slice(11, 16)
            : i.endTime || "11:00",

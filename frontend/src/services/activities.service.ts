...
export const activitiesService = {
  search: async (filters?: ActivityFilters): Promise<Activity[]> => {
    ...
    if (filters?.costMin !== undefined)
      params.set("minCost", String(filters.costMin));
    if (filters?.costMax !== undefined)
      params.set("maxCost", String(filters.costMax));
    ...
    return items.map((item: any) => ({
      ...
      estimatedCost: item.estimatedCost
        ? Number.parseFloat(item.estimatedCost)
        : 35,
      currency: item.currency || "EUR",
      durationMinutes: item.durationMinutes || 90,
      rating: item.rating ? Number.parseFloat(item.rating) : 4.8,
      reviewCount: item.reviewCount || 1250,
      image: cityImageUrl(
        item.location?.name || item.city,
        item.thumbnailUri || item.image,
      ),
      description:
        item.description || `Experience ${item.name} with local guides.`,
    }));
  },
};

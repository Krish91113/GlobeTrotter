...
const config: Record<TripStatus, { label: string; className: string }> = {
  ...
  completed: { label: "Completed", className: "bg-secondary text-muted-foreground" },
};

export function StatusBadge({ status, className }: { status: TripStatus; className?: string }) {
  const c = config[status] ?? config.upcoming;
  return (
    <span className={cn("inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold capitalize", c.className, className)}>
      {c.label}
    </span>
  );
}

  ...
  return (
    <div className="container-page mt-12 space-y-16">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          {trip.cities.map((city, i) => (
            <span key={city} className="flex items-center gap-3">
              ...
              {i < trip.cities.length - 1 && <ArrowRight className="size-4 text-[#94A3B8]" />}
            </span>
          ))}
        </div>
      ...
      {days && days.length > 0 && (
        <section>
          <div className="mb-5 flex items-end justify-between gap-4">
            <h2 className="text-2xl font-bold text-[#0F172A]">Itinerary preview</h2>
            <Link href={`/trips/${tripId}/builder`} className="text-sm font-semibold text-primary hover:underline">
              Edit itinerary
            </Link>
          </div>
                ...
                <div className="mt-3 space-y-2">
                  {day.items.slice(0, 3).map((item) => (
                    <div key={item.id} className="flex items-center gap-3">
                      <span className="w-12 text-sm font-semibold text-[#0F172A]">{item.startTime}</span>
      ...
      {recommendations && recommendations.length > 0 && (
        <section>
          <h2 className="mb-5 text-2xl font-bold text-[#0F172A]">Recommended for your trip</h2>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {recommendations.slice(0, 3).map((rec) => (
              <article key={rec.id} className="overflow-hidden rounded-2xl border border-[#E2E8F0]/60 transition-shadow hover:shadow-md">
                <div className="relative aspect-[4/3] overflow-hidden">
                  <img src={rec.image} alt={rec.activityName} loading="lazy" className="size-full object-cover" />
                  {rec.fitsBudget && (
                ...
                <div className="p-4 space-y-2">
                  <h3 className="font-semibold text-[#0F172A]">{rec.activityName}</h3>

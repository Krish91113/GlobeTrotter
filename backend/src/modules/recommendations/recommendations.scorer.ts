...
export function computeCategoryMatch(
  ...
  const matches = itemCategories.filter((c) => preferred.has(c.toLowerCase())).length;
...
export function scoreCandidate(item: CandidateItem, ctx: ScoringContext): number {
  const categoryMatch = computeCategoryMatch(item.categories, ctx.userCategoryPreferences);
  const budgetFit = computeBudgetFit(item.estimatedCost, ctx.remainingDailyBudget);
  const ratingScore = normalize(item.rating ?? 3.0, 1, 5);
  const popularityScore = normalize(item.ratingCount ?? 0, 0, 10000);
  const diversityBonus = ctx.selectedCategoriesInTrip.includes(item.primaryCategory) ? 0 : 1;

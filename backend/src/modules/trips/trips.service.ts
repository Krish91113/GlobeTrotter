...
async function resolveStatus(code: string): Promise<string> {
  const status = await prisma.tripStatus.findFirst({
    where: { code },
    select: { id: true },
  });
  if (!status) {
    const anyStatus = await prisma.tripStatus.findFirst({
      select: { id: true },
    });
...
async function resolveVisibility(code: string): Promise<string> {
  const vis = await prisma.tripVisibility.findFirst({
    where: { code },
    select: { id: true },
  });
  if (!vis) {
    const anyVis = await prisma.tripVisibility.findFirst({
      select: { id: true },
    });
...
async function toDto(tripId: string): Promise<TripDto> {
  const trip = await prisma.trip.findUnique({
    ...
    include: {
      ...
      stops: {
        include: {
          location: { select: { name: true, normalizedName: true } },
        },
  ...
  if (cities.length === 0) {
    const normalizedTripName = trip.name
      .toLowerCase()
      .replace(/[^a-z0-9]/g, "");
    ...
    const inferred = locations
      .filter((location) =>
        normalizedTripName.includes(
          location.normalizedName.replace(/[^a-z0-9]/g, ""),
        ),
      )
  ...
  const activitiesCount = trip.days.reduce(
    (sum, d) => sum + d.itineraryItems.length,
    0,
  );
  const estimatedSpend = trip.days.reduce(
    (sum, d) =>
      sum +
      d.itineraryItems.reduce(
        (s, i) =>
          s + (i.estimatedCost ? Number.parseFloat(i.estimatedCost.toString()) : 0),
        0,
      ),
    0,
  );
  const totalBudget = trip.budget
    ? Number.parseFloat(trip.budget.targetAmount.toString())
    : 0;
...
export const tripsService = {
  ...
  async createTrip(userId: string, data: CreateTripInput): Promise<TripDto> {
    const currencyId =
  ...
  async updateTrip(
    ...
    const currencyId = data.currency
      ? await resolveCurrency(data.currency)
      : data.defaultCurrencyId !== undefined
